...
- `execution` — implementaciones de `IExecutionEngine` (ADR 0002). Hoy solo existe `NoopExecutionEngine`; `ClineExecutionEngine`/`OpenCodeExecutionEngine` son Fase 7, todavía no empezaron. No agregues una dependencia real de SDK acá sin verificar si esa fase realmente arrancó.
...
- `mcp` — cascarón del package para la Fase 7 (cliente/servidores MCP).
